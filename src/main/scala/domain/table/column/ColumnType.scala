...
object ColumnType {
  ...
  def apply(value: Any): ColumnType = value match {
    ...
    case valueLong: Long => ColumnTypeLong(ColumnLength(valueLong.toString.length))
  ...
  def merge(a: ColumnType, b: ColumnType): ColumnType = {
    (a, b) match {
      ...
      case (ColumnTypeBoolean, ColumnTypeLong(length)) =>ColumnTypeLong(length.max(5)) // 5 = false.toString
      ...
      case (ColumnTypeInt(alength), ColumnTypeLong(blength)) => ColumnTypeLong(alength.max(blength))
      ...
      case (ColumnTypeLong(alength), ColumnTypeLong(blength)) => ColumnTypeLong(alength.max(blength))
      case (ColumnTypeLong(alength), ColumnTypeDouble(blength)) => ColumnTypeDouble(alength.max(blength))
      case (ColumnTypeLong(alength), ColumnTypeString(blength)) => ColumnTypeString(alength.max(blength))
